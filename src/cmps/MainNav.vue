  ...
  <section class="main-nav">
    ...
    <section class="nav">
      ...
      <RouterLink
        class="btn-nav"
        @click="
          $store.commit({ type: 'toggleFilterShown', isFilterShown: true })
        "
        to="/search"
      >
        <div
      ...
      <button class="btn-nav btn-nav-library" @click.stop="onRouterLibrary">
        ...
        <LoginErrModal
          v-if="isLibraryLogErrShown"
          :loginErrType="'library-err'"
          :title="'Enjoy Your Library'"
          :body="'Log in to see saved songs, podcasts, artists, and playlists in Your Library.'"
        />
      </button>
    </section>
    <section class="actions">
      <button @click.stop="onCreateStation" class="create-playlist-btn">
        ...
        <LoginErrModal
          v-if="isCreatePlaylistLogErrShown"
          :loginErrType="'create-playlist-err'"
          :title="'Create a playlist'"
          :body="'Log in to create and share playlists.'"
        />
      </button>
    ...
    <section class="user-playlists">
      <ul class="playlist-links">
        ...
        >
          ...
          <div
            class="played-station-icon"
            v-if="isPlaying && userStation._id === currentStation._id"
            v-html="getSvg('volume100')"
          ></div>
        </RouterLink>
      </ul>
    </section>
